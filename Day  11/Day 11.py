...
starttime = time()
f = list(filter(lambda x:x != '',
    list(open('day11_input.txt').read().split("\n"))))
...
def applyrules(limit=4): 
    ...
    while True:
        for i in coordscop:
            if limit == 4: 
                adjseats = list(tuple(map(sum,zip(i,location))) 
                            for location in mods)
                occupiedseats = len([coordscop[value] for value in adjseats 
